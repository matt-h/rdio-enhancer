...
function injectedJs() {
    ...
    R.enhancer = {
            ...
            R.Models.Playlist.prototype.add = function (model) {
                ...
                if (model_type == "a" || model_type == "al" || model_type == "t" || model_type == "p") {
                    ...
                    if (model_type == "a" || model_type == "al") {
                        ...
                        R.enhancer.getModels(function (tracks) {
                        ...
                        }, model.get("tracks"), 'Fetching Playlist data... Please wait. If your Playlist is large this can take awhile.', 'There was an error getting the Playlist data, if you have a large Playlist try scrolling down to load more first and then try the action again.');
                        return;
                    }
                    if (playlist_this.has("tracks")) {
                        playlist_this.get("tracks").addSource(model);
            ...
            R.Component.create = function (a, b, c) {
                ...
                if (a == "Dialog.EditPlaylistDialog.Rdio") {
                    b._getAttributes = function () {
                        ...
                        if (this.model.isNew()) {
                            var track_list = "", source_model = this.options.sourceModel;
                            if (source_model) {
            ...
            switch (R.enhancer.get_setting("notifications")) {
                ...
                case "chrome":
                    if ("Notification" in window) {
                        ...
                        var notification = webkitNotifications.createNotification("", "Rdio Notification", msg_txt);
                        notification.show();
                    }
                    break;
            }
        ...
        getTracks: function (callback) {
            R.enhancer.getModels(callback, R.enhancer.current_playlist.model.get("tracks"), 'Fetching playlist data... Please wait. If your playlist is long this can take awhile.', 'There was an error getting the playlist data, if you have a long playlist try scrolling down to load more first and then try the action again.');
        },
        ...
        sortByAlbum: function (a, b) {
            var album_a = a.attributes.source.attributes.album.toLowerCase(), album_b = b.attributes.source.attributes.album.toLowerCase();
            if (album_a < album_b) {
                return -1;
            }
        ...
        sortByReleaseDate: function (a, b) {
            var date_a = a.attributes.source.attributes.releaseDate, date_b = b.attributes.source.attributes.releaseDate;
            if (date_a < date_b) {
                return -1;
            }
        ...
        sortByPlayCount: function (a, b) {
            var count_a = a.attributes.source.attributes.playCount, count_b = b.attributes.source.attributes.playCount;
            if (count_a < count_b) {
                return 1;
            }
        ...
        sortByTrackName: function (a, b) {
            var trackname_a = a.attributes.source.attributes.name.toLowerCase(), trackname_b = b.attributes.source.attributes.name.toLowerCase();
